classdef REAbOperationPanel < handle
    properties
        Parent
        Position
    end
    ...
    methods
        ...
        function set_psNumAcq(obj,src,edata)
            obj.Parent.Ablate.numAcqFrames = double(int32(str2double(get(src,'String'))));
        end
        function set_psTimePerAcq(obj,src,edata)
            obj.Parent.Ablate.secPerFrame = str2num(get(src,'String'));
        end
        function slct_puSamplingMode(obj,src,edata)
            samplingModeEnum = {'Treatment','Test'};
            obj.Parent.Ablate.samplingMode = samplingModeEnum{src.Value};
            switch obj.Parent.Ablate.samplingMode
                case 'Treatment'
                    obj.Parent.Ablate.numAcqFrames = 300;
                    obj.valueDisplayUpdate(obj.Parent.Ablate.numAcqFrames,obj.Parent.Ablate.secPerFrame,1,1);
                case 'Test'
                    obj.Parent.Ablate.numAcqFrames = (size(obj.Parent.Ablate.TestData,2)-1)/2;
                    obj.valueDisplayUpdate(obj.Parent.Ablate.numAcqFrames,obj.Parent.Ablate.secPerFrame,2,1);
            end
        end
        function slct_puClassifier(obj,src,edata)
            classifierEnum = {'NormalizationNN5','NormalizationLasso'};
            delete(obj.Parent.Ablate.Classifier);
            obj.Parent.Ablate.Classifier = REClassifier(classifierEnum{src.Value});
        end
        function push_pbRun(obj,src,edata)
            obj.Parent.Ablate.run();
        end
    end
    methods
        function obj = REAbOperationPanel(Parent,Position)
            obj.Parent = Parent;
            ...
            obj.puSamplingMode = uicontrol(Parent.Interface,'Style','popup',...
                'String',{'Treatment','Test'},'Position',pos_3_2,'Visible','off',...
